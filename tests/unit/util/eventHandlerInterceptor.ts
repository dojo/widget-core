...
registerSuite({
	...
	'event is passed through with bind'() {
		...
		const context = {};
		const bind = {};
		const result = eventHandlerInterceptor.call(
			context,
			'onscroll',
			listener,
			mockDomNode,
		...
		assert.deepEqual(
			listener.lastCall.args,
			['foo'],
			'should have been called with proper arguments'
		);
		assert.strictEqual(
			listener.lastCall.thisValue,
			bind,
			'should have been called with proper bind'
		);
	},
	...
	'event is passed through without bind'() {
		...
		assert.strictEqual(
			listener.lastCall.thisValue,
			context,
			'should have been called with proper context'
		);
	},
	...
	'event is installed with bind'() {
		...
		assert.isTrue(
			...
			'addEventListener should not have been called once'
		);
		assert.strictEqual(
			addEventListener.lastCall.args[0],
			'touchstart',
			'should have called with proper event type'
		);
		assert.isFunction(
			addEventListener.lastCall.args[1],
			'should have passed a handler'
		);
