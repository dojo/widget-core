...
class TestWidget extends ThemeableMixin(WidgetBase)<
	ThemeableProperties<typeof baseThemeClasses1>
> {}
...
@theme(baseThemeClasses1)
@theme(baseThemeClasses2)
class StackedTestWidget extends ThemeableMixin(WidgetBase)<
	ThemeableProperties
> {}

@theme(baseThemeClasses3)
@theme(baseThemeClasses1)
class DuplicateThemeClassWidget extends ThemeableMixin(WidgetBase)<
	ThemeableProperties
> {}

class NonDecoratorDuplicateThemeClassWidget extends ThemeableMixin(WidgetBase)<
	ThemeableProperties
> {
	constructor() {
		super();
...
registerSuite({
	...
	'classes function': {
		...
		'should ignore any new classes that do not exist in the baseThemeClasses1 and show console error'() {
			...
			const flaggedClasses = themeableInstance
				.classes(class1, newClassName)
				.get();
			...
			assert.strictEqual(
				...
				`Class name: ${newClassName} not found, use chained 'fixed' method instead`
			);
		},
		...
		'should filter out falsy params passed to classes function'() {
			...
			const { class1, class2 } = baseThemeClasses1;
			const flaggedClasses = themeableInstance
				.classes(class1, null, class2, null, '')
				.get();
			assert.deepEqual(flaggedClasses, {
				[baseThemeClasses1.class1]: true,
				[baseThemeClasses1.class2]: true
	...
	'classes.fixed chained function': {
		...
		'should negate any new classes that are not requested on second call'() {
			...
			const { class1 } = baseThemeClasses1;
			const fixedClassName = 'fixedClassName';
			const flaggedClassesFirstCall = themeableInstance
				.classes(class1)
				.fixed(fixedClassName)
				.get();
			assert.deepEqual(
				flaggedClassesFirstCall,
				{
					[baseThemeClasses1.class1]: true,
					[fixedClassName]: true
				},
		...
		'should remove adjoined fixed classes when they are no longer provided'() {
			...
			const flaggedClassesSecondCall = themeableInstance
				.classes(class1, class2)
				.get();
			assert.deepEqual(
				flaggedClassesSecondCall,
	...
	'setting base theme classes': {
		decorator: {
			...
			'warns on clashing class names and uses the latest applied'() {
				...
				assert.strictEqual(
					consoleStub.firstCall.args[0],
					`Duplicate base theme class key 'class1' detected, this could cause unexpected results`
				);
			}
		},
		'non decorator': {
	...
	'injecting a theme': {
		'theme can be injected by defining a ThemeInjector with registry'() {
			...
			const themeableInstance = createTestWidget(InjectedTheme, {
		...
		'setting the theme invalidates all "Themeable" widgets and the new theme is used'() {
			const themeInjectorContext = registerThemeInjector(
				testTheme1,
				testRegistry
			);
			class InjectedTheme extends TestWidget {
