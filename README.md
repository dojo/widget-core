...
Dojo 2 widgets are styled and themed using the `ThemeableMixin` from `@dojo/widget-core/mixins/Themeable`.



Css-modules allow us to write our css classes without worrying about styles leaking out into other components or classnames clashing.

To use the Themeable mixin, a widget must implement a `.m.css` file that contains all the css classes that will be used to style it. This `.m.css` file must be imported and passed to the `@theme` decorator provided by the mixin.
...
The css that is passed to the `@theme` decorator allows the widgets css classes to be themed via the dojo 2 theming system.
Themeable mixin provides a `this.classes` function which should be used within the `render` function to apply both themeable and fixed classes to the `VDom` it returns.
...
#### Authoring a widgets css file

A widgets css file should have a `.m.css` extension to mark it to our build system as a css-module file. It should be located in a `styles` directory alongside the widget code itself.

The css file should contain the complete set of css classes required to style the widget. This should consist of both themeable and fixed classes. The latter classes should be notated via a `Fixed` suffix. The root node of each widget should be given a `root` classname for consistency.
...
#### Applying classes to a widget

Classes are applied to VNodes using the `this.classes` function that is provided by the `ThemeableMixin`. Classes passed to the `this.classes` function will be replaced with and provided `theme` classes at run time. To specify fixed structural styles to your VNodes that you do not wish to be themed you can pass them via the `.fixed()` function returned from the initial `this.classes` call.

``` typescript
// TabPanel.ts
import * as css from './styles/tabPanel.m.css';
import { ThemeableMixin, theme } from '@dojo/widget-core/mixins/Themeable';

@theme(css)
export default class TabPanel extends ThemeableMixin(WidgetBase) {
	render() {
		return v('div', { classes: this.classes(css.root).fixed(css.rootFixed) }, [
			v('div', { classes: this.classes(css.tab) })
		]);
	}
}
```

In the above example, the `rootFixed` class will always apply and cannot be themed but the `root` and `tab` classes can.

#### Writing a theme

Themes are typescript modules that export an object that contains css-modules files keyed by widget name. Dojo 2 widget theme keys are generated by their `.m.css` filename are prefixed by `dojo-` and . For example to target the above `tabPanel.m.css` classes, the required theme key is `dojo-tabPanel`. When targeting your own custom widgets, the `dojo-` prefix is not required.

``` css
/* myTheme/styles/tabPanel.m.css */
.root {
	color: blue;
}
```
...
``` typescript
// myTheme/theme.ts
import * as tabPanel from './styles/tabPanel.m.css';

export default {
	'dojo-tabPanel': tabPanel
}
```

In the above example, the new `root` class provided by the theme for `dojo-tabPanel` will replace the `root` class that was provided by the original `tabPanel.m.css`. This means the tabPanel will now have color set to blue, but will no longer receive a red background as was set in the original file.
